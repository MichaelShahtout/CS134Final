...
Box Octree::meshBounds(const ofMesh & mesh) {
	...
	//cout << "vertices: " << n << endl; // i uncommented
...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
    ...
    vector<Box> boxList;
    vector<int> returnedPts;
    
    subDivideBox8(node.box,boxList);
    for(int i = 0; i < boxList.size(); i++ ){
        TreeNode child;
        
        // Check to see if there are points in it, the local treenode will get added to the current node's children if the child has at least 2 or more
        if(getMeshPointsInBox(mesh, node.points, boxList.at(i), returnedPts) > 0 ){
            
            child.points = returnedPts;
            //If yes, then assign child node that box and the points inside of that box then add that  child to the current node's list of children
            child.box = boxList.at(i);
            node.children.push_back(child); // add it to the main tree
            //Pass in the child node you just pushed_back'd, not the local child treeNode
            if(getMeshPointsInBox(mesh, node.points, boxList.at(i), returnedPts) >= 2){
